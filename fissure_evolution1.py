...
#ON TRAVAILLE ICI AVEC DES PNG
#La conversion de fichiers bmp en png avec de plus faibles résolutions peut s’avérer utile
...
def detection_sobel(path):
    ...
    # on le stocke dans un fichier intermediaire
    cv.imwrite("/Users/charlesassoi/opt/anaconda3/pkgs/imagecodecs-2021.8.26-py39ha952a84_0/info/recipe/inter.jpg", img)
    grad = main(["/Users/charlesassoi/opt/anaconda3/pkgs/imagecodecs-2021.8.26-py39ha952a84_0/info/recipe/inter.jpg"])
    ...
    params.minArea = 0.000000000000000000000000000000001

    # Set Circularity filtering parameters
    params.filterByCircularity = True
    params.minCircularity = 0.00000000000000000009

    # Set Convexity filtering parameters
    params.filterByConvexity = True
    params.minConvexity = 0.000000000000009

    # Set inertia filtering parameters
    params.filterByInertia = True
    params.minInertiaRatio = 0.000000000000000001
...
#detection_sobel("/Users/charlesassoi/Downloads/Mode 1_2022/002295.png")
...
#print(liste_tri_haut("/Users/charlesassoi/Downloads/000099 Small 2.png", 10))
...
#print(liste_tri_bas("002295 Small.png", 0.0000000001))
...
def barycentre(path,ecart_points):
    ...
    liste=(y-z)/((y-z)[0])
    print(liste)
    #mettre le return en commentaire temporairement pour l'affichage
    return liste
    im = cv.imread(path)#superposition de l'image
    im = im[50:230, 50:230]
    im = cv.resize(im, (500, 1000))
    cv.imwrite("/Users/charlesassoi/PycharmProjects/TensorEnv/inter_im.jpg", im)
    im = plt.imread("/Users/charlesassoi/PycharmProjects/TensorEnv/inter_im.jpg")
    plt.subplots()
    plt.imshow(im)
    plt.plot(x, y, "y-")
    plt.plot(x,z,"y+")
    ...
    plt.title('barycentres')
    plt.grid()
    plt.show()

#barycentre("/Users/charlesassoi/Downloads/Mode 1_2022/000111.png",10)
def fissure(path,ecart_points):
    ...
    yp.reverse()
    x = np.linspace(np.min(xp), np.max(xp), len(l))
    if  liste[-1]/liste[0]>1.5:
        while abs(liste[i - 1] - liste[i]) < 0.25:
        #print(abs(liste[i+1]/liste[i]))
            i += 1
    else:
        while abs(liste[i - 1] - liste[i]) < 0.11:
        #print(abs(liste[i+1]/liste[i]))
            i += 1
    print("le front de fissure se situe entre la {}èmeet la {}ème mire".format(i, i + 1))
    # Position fissure
    a1 = (yp[i - 1] - zp[i]) / (x[i - 1] - x[i])
    a2 = (zp[i - 1] - yp[i]) / (x[i - 1] - x[i])
    b1 = yp[i] - a1 * x[i]
    b2 = zp[i] - a2 * x[i]
    f1 = lambda x: a1 * x +b1
    f2 = lambda x: a2 * x + b2
    ...
    def dichotomie(g, a, b, epsilon):
        ...
        while (abs(b - a) > epsilon):
            ...
            if (g(a) * g(b) <= 0):
                ...
                a = c
        print("l'abscisse du front de fissure est " + str((b + a) / 2) + " et son ordonnée est " + str(f1((b + a) / 2)))
        return (b + a) / 2

    u = dichotomie(g, x[i - 1], x[i], 0.00001)
    return ([u, f1(u)])
    #Pour l'affichage on met le return en commentaire temporairement
    im = cv.imread(path)
    im = im[50:230, 50:230]
    im = cv.resize(im, (500, 1000))
    cv.imwrite("/Users/charlesassoi/PycharmProjects/TensorEnv/inter_im.jpg", im)
    ...
    liste1=[x[i-1],x[i]]
    plt.plot(liste1, [f1(x[i - 1]),f1(x[i])], "r-")
    plt.plot(liste1, [f2(x[i - 1]),f2(x[i])], "b-")
    #plt.plot(u, f1(u), "b+")
    plt.xlabel('coordonnée x')
    plt.ylabel('coordonnée y')
...
#fissure("/Users/charlesassoi/Downloads/Mode 1_2022/002295.png",10)
...
def fissure_serie(path_dossier,ecart_points):
    ...
    fis = []
    for i in listeFichiers[1:]:
        fis.append(fissure(path_dossier+"/{}".format(i), ecart_points))
    print(fis)
    print(listeFichiers)
    return(fis)

#fissure_serie("/Users/charlesassoi/Downloads/Mode 1_2022",10)
...
def ligne_fissure(path_dossier,ecart_points):
    ...
    plt.xlabel('coordonnée x')
    plt.ylabel('coordonnée y')
    plt.title('propagation fissure')
    plt.grid()
    plt.show()
ligne_fissure("/Users/charlesassoi/Downloads/Mode 1_2022",10)